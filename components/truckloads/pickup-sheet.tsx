...
export function PickupSheet({ 
  ...
  useEffect(() => {
    async function fetchPickupStops() {
      try {
        setIsLoading(true)
        const response = await fetch(`/api/truckloads/${truckloadId}/orders`)
        if (!response.ok) {
          throw new Error("Failed to fetch pickup stops")
        }
        const data = await response.json()
        if (!data.success) {
          throw new Error(data.error || "Failed to fetch pickup stops")
        }
        
        // Filter for pickup stops only and sort by sequence number (bottom to top)
        const pickupStops = data.orders
          .filter((stop: any) => stop.assignment_type === 'pickup')
        ...
        console.error("Error fetching pickup stops:", err)
      } finally {
        setIsLoading(false)
      }
    }
      ...
      <Card className="w-full">
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="text-2xl font-bold">Pickup Sheet</CardTitle>
  ...
  if (error) {
    return (
      <Card className="w-full">
        <CardHeader>
          <CardTitle className="text-2xl font-bold text-red-500">Pickup Sheet</CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <CardHeader className="pb-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <CardTitle className="text-3xl font-bold text-gray-900">Pickup Sheet</CardTitle>
