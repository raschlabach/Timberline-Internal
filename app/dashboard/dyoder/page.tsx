...
import { ImportDetail } from '@/components/dyoder/import-detail'

interface DYoderImport {
  id: number
  file_name: string
...
export default function DYoderPage() {
  const [imports, setImports] = useState<DYoderImport[]>([])
  ...
  const fetchImports = useCallback(async () => {
    try {
      const res = await fetch('/api/dyoder/imports')
  ...
  async function uploadFile(file: File) {
    ...
    try {
      ...
      const res = await fetch('/api/dyoder/upload', {
        method: 'POST',
        body: formData,
      })
  ...
  async function handleDelete(id: number, e: React.MouseEvent) {
    ...
    try {
      const res = await fetch(`/api/dyoder/imports/${id}`, { method: 'DELETE' })
      if (!res.ok) {
        const data = await res.json()
  ...
  async function handleBulkStatusUpdate(status: 'hidden' | 'active') {
    ...
    try {
      const res = await fetch('/api/dyoder/imports/bulk-status', {
        method: 'POST',
    ...
    <div className="max-w-[1400px] mx-auto">
      <div className="mb-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">D. Yoder Hardwoods</h1>
            <p className="text-sm text-gray-500 mt-1">
              Upload shipment batch reports from D. Yoder Hardwoods and convert them into delivery orders
            </p>
          </div>
        </div>
      ...
      <Tabs value={activeTab} onValueChange={(v) => { setActiveTab(v); setSelectedImports(new Set()) }}>
        ...
        <TabsContent value="active">
          <ImportsList
            ...
            emptyDescription="Upload a D. Yoder Hardwoods Excel file to get started"
        ...
        <TabsContent value="past">
          <ImportsList
            ...
            emptyDescription="Imports move here once all items are converted"
            selectedImports={selectedImports}
            onToggleSelect={(id, checked) => {
              setSelectedImports(prev => {
                const next = new Set(prev)
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b">
                ...
                <th className="px-4 py-2.5 text-left font-medium text-gray-500">Batch</th>
                <th className="px-4 py-2.5 text-left font-medium text-gray-500">State</th>
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 font-medium">
                      {imp.batch_number ? `Batch #${imp.batch_number}` : imp.file_name}
                    </td>
                    <td className="px-4 py-3 text-gray-500">{imp.ship_to_state || '—'}</td>
