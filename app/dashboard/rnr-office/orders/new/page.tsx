...
import { ArrowLeft, Plus, Trash2, Search, Loader2, AlertTriangle, Save, ClipboardList, Sparkles } from 'lucide-react'
...
export default function NewOrderPage() {
  ...
  const searchParts = useCallback(async (index: number, q: string) => {
    if (q.length < 1) {
      updateItem(index, { partResults: [], showDropdown: false, isSearching: false })
      return
    }
  ...
  function markNewPart(index: number) {
    updateItem(index, {
      is_new_part: true,
      showDropdown: false,
      partResults: [],
  ...
  async function createPartAndLink() {
    ...
    try {
      const body: Record<string, unknown> = {
        ...
        item_class: newPartForm.item_class || null,
        qb_item_code: newPartForm.qb_item_code || null,
      }
      ...
      if (res.ok) {
        ...
        toast.error(d.error || 'Failed to create part')
      }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">Order number will be auto-generated on save</p>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="flex items-end">
            ...
            >
              ...
              {isRush ? 'RUSH ORDER' : 'Mark as Rush'}
            </Button>
          </div>
        </div>
      </div>
                    ...
                    {item.showDropdown && item.partSearch.length >= 2 && item.partResults.length === 0 && !item.isSearching && (
                      <div className="absolute z-50 mt-1 left-3 right-3 bg-white border border-gray-200 rounded-lg shadow-lg p-3 text-center">
                        ...
                        <Button
                          ...
                          onMouseDown={() => { markNewPart(idx); openNewPartDialog(idx) }}
                        >
      ...
      <div className="flex items-center justify-end gap-3 pb-8">
        ...
        <Button className="gap-1.5 bg-amber-600 hover:bg-amber-700" onClick={() => submitOrder(false)} disabled={isSubmitting}>
          ...
          <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-1">
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Input value={newPartForm.rnr_part_number} onChange={e => setNewPartForm({ ...newPartForm, rnr_part_number: e.target.value })} className="font-mono text-sm" />
              </div>
              <div>
                <Label className="text-xs">Customer Part #</Label>
                <Input value={newPartForm.customer_part_number} onChange={e => setNewPartForm({ ...newPartForm, customer_part_number: e.target.value })} className="font-mono text-sm" />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
              ...
              <div>
                ...
                <Input type="number" step="0.0001" value={newPartForm.price} onChange={e => setNewPartForm({ ...newPartForm, price: e.target.value })} />
              </div>
            </div>
            <div>
              <Label className="text-xs">Description</Label>
              <Input value={newPartForm.description} onChange={e => setNewPartForm({ ...newPartForm, description: e.target.value })} />
            </div>
            ...
            <div className="grid grid-cols-5 gap-3">
              <div>
                ...
                <Input type="number" step="any" value={newPartForm.thickness} onChange={e => handleNewPartDimChange('thickness', e.target.value)} className="font-mono text-sm" />
              </div>
              <div>
                <Label className="text-xs">Width</Label>
                <Input type="number" step="any" value={newPartForm.width} onChange={e => handleNewPartDimChange('width', e.target.value)} className="font-mono text-sm" />
              </div>
              <div>
                <Label className="text-xs">Length</Label>
                <Input type="number" step="any" value={newPartForm.length} onChange={e => handleNewPartDimChange('length', e.target.value)} className="font-mono text-sm" />
              </div>
              <div>
                <Label className="text-xs">Board Ft <span className="text-amber-600">(auto)</span></Label>
                <Input type="number" step="any" value={newPartForm.board_feet} readOnly className="font-mono text-sm" />
              </div>
              <div>
                <Label className="text-xs">Lineal Ft <span className="text-amber-600">(auto)</span></Label>
                <Input type="number" step="any" value={newPartForm.lineal_feet} readOnly className="font-mono text-sm" />
              </div>
            </div>
          </div>
