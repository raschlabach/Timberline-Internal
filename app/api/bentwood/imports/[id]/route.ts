...
export async function GET(
  ...
  try {
    ...
    const importResult = await query(
      `SELECT
        i.id, i.file_name, i.week_label, i.week_date,
        i.total_items, i.total_skids, i.total_bundles,
        i.status, i.notes, i.created_at,
        u.full_name as created_by_name
      FROM bentwood_imports i
    ...
    const itemsResult = await query(
      `SELECT
        ...
        TO_CHAR(item.pickup_date, 'YYYY-MM-DD') as pickup_date_formatted,
        TO_CHAR(item.delivery_date, 'YYYY-MM-DD') as delivery_date_formatted
    ...
    console.error('Error fetching Bentwood import:', error)
    return NextResponse.json(
...
export async function DELETE(
  ...
  try {
    ...
    const convertedCheck = await query(
      `SELECT COUNT(*) as count FROM bentwood_import_items
    ...
    await query('DELETE FROM bentwood_imports WHERE id = $1', [importId])

    return NextResponse.json({ success: true })
  } catch (error) {
    console.error('Error deleting Bentwood import:', error)
    return NextResponse.json(
