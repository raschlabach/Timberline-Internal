 ...
 * 
 * Usage:
 *   node scripts/import-customers-debug.js [filename] [branch]
 * 
 * Where:
    ...
    try {
      ...
      if (isFirstRecord) {
        ...
        const continueImport = await new Promise((resolve) => {
          rl.question('Do you want to continue with the import based on the above field names? (y/N) ', (answer) => {
      ...
      try {
        ...
        // Debug which phone fields are being used
        const phone1 = record.phone_number1 || record.phone1 || record.phone_number_1 || record['phone number 1'] || '';
        const phone2 = record.phone_number2 || record.phone2 || record.phone_number_2 || record['phone number 2'] || null;
        
        // Debug notes field
        const notes = record.notes || record.note || record.Notes || record.comments || record.Comments || null;
        ...
        await client.query(
          ...
          [
            record.customer_name,
            locationId,
            phone1.toString().replace(/\D/g, ''),
            phone2 ? phone2.toString().replace(/\D/g, '') : null,
            notes
          ]
        );
